...
| **Gemini CLI**  | OAuth, API key        | Experimental | `add-account`, `switch-account`, `delete-account`, `launch-tool` | OAuth accounts isolate a retained managed `GEMINI_CLI_HOME`; the vendor writes `oauth_creds.json`. Selection sets child-only `GEMINI_CLI_HOME` and `GOOGLE_GENAI_USE_GCA`. API-key accounts still use `CredentialStore` and leave the configuration tree unchanged. Forgetting OAuth metadata retains the vendor home. `GEMINI_CLI_HOME` and `GOOGLE_GENAI_USE_GCA` are `[verified-source]`; a signed-in host observation is still missing. | Empty snapshot vector. Published limits do not establish current utilization.                      |
...
| **Gemini CLI**  | Managed API keys in `CredentialStore`; selected OAuth homes via `GEMINI_CLI_HOME` `[verified-source]`; default `~/.gemini/oauth_creds.json`, `google_accounts.json`, and `settings.json` are `[verified-source]`, but were not observed on a signed-in host | API-key mode `[verified-docs]`; `oauth-personal` `[verified-source]`       |
...
4. **Gemini CLI fourth.** The API-key path meets its M3 exit criterion. OAuth
   add/select/launch now isolates `GEMINI_CLI_HOME` from `[verified-source]`
   environment semantics. A signed-in host observation is still missing, and a
   same-home OAuth file-swap switch remains out of bounds.
...
| Is replacing Gemini `oauth_creds.json`, with coordinated `google_accounts.json` and settings state, sufficient for a next-process switch? | A same-home OAuth file-swap switch. Isolated `GEMINI_CLI_HOME` add/select/launch is already implemented. |
