...
| --------------- | ----------------------------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| **Codex CLI**   | OAuth (ChatGPT plan), API key | `~/.codex/auth.json` `[verified-local]`                                            | Plain JSON file `[verified-local]`                          | Implemented as live-home `auth.json` replacement. Local `login status` follows that file `[verified-local]`. Vendor acceptance of a copied credential is untested `[unknown]`. The adapter does not switch by relocating `CODEX_HOME`. | Not observed locally `[unknown]`                                                                   | Yes, OpenAI-compatible base URL `[verified-docs]`                          | **Low** — one document to swap, and a second env-var strategy as backup         |
...
Codex CLI can add a stored account, list the live identity together with
those stored copies, switch by replacing `auth.json`, and delete a stored
copy without signing the tool out. Local `login status` follows that file
`[verified-local]`. Vendor acceptance of a copied credential remains
untested (`docs/research/codex-cli.md` §5). Grok CLI lists signed-in OIDC
identities, skips reserved scopes, and honours `$GROK_HOME`. Claude Code
lists the on-disk identity by reading `.credentials.json` and
`~/.claude.json`. Gemini CLI lists an account when `GEMINI_API_KEY` is set.
It does not list OAuth accounts: the adapter still reads only
`GEMINI_API_KEY`, not the OAuth files vendor source now names. No adapter
other than Codex CLI implements switching. Grok CLI has no in-file
