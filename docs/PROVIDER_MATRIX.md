...
| **Gemini CLI**  | Managed API keys in `CredentialStore`; isolated OAuth homes under the application data directory write `oauth_creds.json` and `google_accounts.json`. Live `~/.gemini` paths remain `[verified-source]` and were not observed on a signed-in host | API-key mode `[verified-docs]`; OAuth launch uses `GEMINI_CLI_HOME` `[verified-source]` |
...
4. **Gemini CLI fourth.** The API-key path meets its M3 exit criterion.
   Isolated-home OAuth add is implemented. Live-home OAuth file-swap remains
   unimplemented.
