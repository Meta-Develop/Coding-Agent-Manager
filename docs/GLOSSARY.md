...
| **Maturity**                  | How complete an adapter is: `planned`, `experimental`, or `supported`. Shown in the UI so it cannot overstate itself. |
...
| **Switch verification**       | Confirming after a switch that the tool reports the expected identity. A switch is not complete until this passes.    |
