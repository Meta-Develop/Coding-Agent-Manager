...
//!
//! OAuth add/select/launch isolates a managed `GEMINI_CLI_HOME` and lets the
//! vendor `gemini` process write `oauth_creds.json`. This adapter never
//! replaces `oauth_creds.json`, `google_accounts.json`, or `settings.json`.
//! The effective auth-mode gate follows the pinned first-party source in
...
pub struct GeminiCliAdapter {
    ...
    /// Test seam for interactive Google sign-in. Production inherits stdio
    /// and starts the vendor `gemini` process itself.
...
impl GeminiCliAdapter {
    ...
    /// Complete an isolated Google sign-in without starting a real browser.
    ...
    fn run_login(&self, managed_home: &Path) -> Result<()> {
        let status = if let Some(runner) = self.login_runner {
            runner(managed_home).map_err(|error| {
                config_write(format!("gemini login could not start ({})", error.kind()))
            })?
        } else {
            let mut command = Command::new(self.program());
            command
                .current_dir(managed_home)
                .env(HOME_ENV, managed_home)
                .env(GCA_ENV, "true");
        ...
        if status != 0 {
            return Err(config_write(format!(
                "gemini login exited unsuccessfully (status {status})"
            )));
        }
        Ok(())
    }
...
impl ProviderAdapter for GeminiCliAdapter {
    ...
    fn launch_spec(&self, account: &StoredAccountMetadata) -> Result<LaunchSpec> {
        ...
        if account.auth_kind == AuthKind::OAuth {
            ...
            let mut spec = LaunchSpec::new(self.program())
                .current_dir(cwd)
...
/// Mask `google_accounts.json` `active` in the spec shape `a***@example.com`.
///
/// `old` is history and is never listed as a second live account. The raw
/// email never appears in the returned value.
...
mod tests {
    ...
    const TEST_OAUTH_BYTES: &[u8] = b"FAKE-gemini-oauth-creds-0001";
    ...
    fn write_oauth_creds(home: &Path) -> io::Result<i32> {
        fs::create_dir_all(home.join(".gemini"))?;
        fs::write(home.join(".gemini/oauth_creds.json"), TEST_OAUTH_BYTES)?;
        Ok(0)
    }

    fn write_oauth_creds_and_accounts(home: &Path) -> io::Result<i32> {
        write_oauth_creds(home)?;
        fs::write(
            home.join(".gemini/google_accounts.json"),
            r#"{"active":"FAKE-user-0001@example.invalid","old":[]}"#,
        )?;
        Ok(0)
    }

    fn fail_login(_home: &Path) -> io::Result<i32> {
        Ok(2)
    }

    fn empty_success_login(_home: &Path) -> io::Result<i32> {
        Ok(0)
    }
    ...
    fn oauth_adapter(runner: LoginRunner) -> (tempfile::TempDir, GeminiCliAdapter) {
        let (dir, adapter) = context(None);
        (dir, adapter.with_login_runner(runner))
    }
    ...
    #[test]
    fn oauth_login_runner_lists_complete_account_without_leaking() {
        let (_dir, adapter) = oauth_adapter(write_oauth_creds);
    ...
    fn oauth_active_email_is_masked_and_old_is_not_a_second_account() {
        let (_dir, adapter) = oauth_adapter(write_oauth_creds_and_accounts);
        add_managed_account_for(
            &registry_for(&adapter),
            &adapter,
            "work",
    ...
    #[test]
    fn oauth_login_failure_or_missing_creds_is_not_complete() {
        let (_dir, adapter) = oauth_adapter(fail_login);
        ...
        assert_no_fake("login failure", &format!("{error} {error:?}"));
        ...
        let (_dir, adapter) = oauth_adapter(empty_success_login);
        let error = add_managed_account_for(
            &registry_for(&adapter),
            &adapter,
            "missing",
    ...
    #[test]
    fn oauth_recovery_revalidates_without_a_second_login() {
        let (dir, adapter) = oauth_adapter(panic_login);
        ...
        ensure_private_managed_home(&adapter.resolved_data_dir().expect("data dir"), &home)
            .expect("home");
        write_oauth_creds(&home).expect("existing creds");
        let _ = dir;
        adapter
    ...
    #[test]
    fn oauth_login_creates_only_the_managed_directory() {
        let (_dir, adapter) = oauth_adapter(write_oauth_creds);
        let home = managed_account_dir(
            &adapter.resolved_data_dir().expect("data dir"),
            PROVIDER_ID,
        ...
        assert!(!google_accounts_path(&home).exists());
    }
    ...
    fn oauth_launch_refuses_conflicting_or_external_auth() {
        let (dir, adapter) = oauth_adapter(write_oauth_creds);
        add_managed_account_for(
            &registry_for(&adapter),
            &adapter,
            "work",
